...
Windows定义的对象种类是有限的，对于每种对象各有一个全局的`POBJECT_TYPE`变量指向其类型对象。Windows内部有一个`_OBJECT_TYPE`类型的结构体数组，对象头通过`TypeIndex`成员来关联对应的对象类型。接下来使用WRK的源码作为例子来描述Windows创建一个对象类型的过程，虽然WRK与现在的Windows有着一定的差别，不过一些设计理念仍然得到了保留。
...
Windows使用了`ObCreateObjectType`来新建一个对象。下面展示了`ObCreateObjectType`函数的原型：
...
对`TypeName`, `ObjectTypeInitializer`进行检查。如果是无效的类型就返回错误。接着Windows确保类型名字中并不包含路径分隔符，如果类型名字已经存在了对应的对象类型目录，就报错。接下来，Windows会使用`ObpAllocateObject`为该类型分配一个对象，该对象的对象体为`_OBJECT_TYPE`。
...
接着，Windows初始化这个对象。如果此时并没有一个该对象类型的对象类型，那么就让该对象类型成为该对象类型。设置新类型的对象类型的Key成员，该成员是一个四字节的数字(其实为4个ASCII码字符)
...
最后，如果没有该对象类型的目录，就将该改对象插入到根目录当中，设置输出对象类型，返回成功。如果已经有了该对象类型的目录，就引用该对象，接着设置输出对象类型，返回成功。否则，释放根目录并且返回失败。
